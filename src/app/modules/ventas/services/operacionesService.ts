 ...
 * @param factura
 */
export const genCalculoTotalesService = (factura: FacturaInputProps): { subTotal: number, total: number, montoPagar: number, vuelto: number } => {
    const subTotal = montoSubTotal(factura)
    const total = subTotal - factura.descuentoAdicional
    const montoPagar = subTotal - genReplaceEmpty(factura.descuentoAdicional, 0) - genReplaceEmpty(factura.montoGiftCard, 0)
