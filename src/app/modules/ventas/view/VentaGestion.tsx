...
const tableColumns: MRT_ColumnDef<FacturaProps>[] = [
    ...
    },
    {
        ...
        Cell: ({cell}) => (
            <span>{numberWithCommas((cell.getValue() as number), {})}</span>
        ),
        size: 50,
    ...
    },
    {
        ...
        accessorFn: (row) => (
            <span>{row.cliente.numeroDocumento} {row.cliente.complemento?`-${row.cliente.complemento}`:''}</span>
        ),
    ...
    },
    {
        ...
        accessorFn: (row) => (
            <Chip
                color={row.state === apiEstado.validada ? 'success' :
                    row.state === apiEstado.pendiente ? 'warning' : 'error'}
                label={row.state}
                size={'small'} />
        ),
        filterFn: (row, id, filterValue) =>
