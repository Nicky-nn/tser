...
export const productoRegistroVarianteValidatorioSchema = {
    ...
    inventario: array().of(
        object({
            sucursal: object().shape({
                codigo: number().integer().required()
            }).required(),
            stock: number().min(0).nullable(),
        })
    ),
    unidadMedida: object().nullable().default({}).shape({
 ...
 * @param prod
 */
export const productoRegistroValidator = async (prod: ProductoInputProps): Promise<Array<string>> => {
    try {
        setLocale(es)
        ...
        if (prod.varianteUnica) {
            ...
            for (const variante of prod.variantes) {
                ...
                if (!variante.unidadMedida) {
                    throw new Error(`Variante ${variante.codigoProducto}: Debe seleccionar la Unidad de medida`)
                }
                if (variante.precioComparacion || 0 > 0) {
                    if (variante.precio > variante.precioComparacion!) {
                        throw new Error(`Variante ${variante.codigoProducto}: El precio de comparación debe ser mayor al precio Original`)
                    }
                }
                if (variante.costo > 0) {
                    if (variante.costo > variante.precio) {
                        throw new Error(`Variante ${variante.codigoProducto} El precio debe ser mayor al costo`)
                    }
                }
            }
        }
