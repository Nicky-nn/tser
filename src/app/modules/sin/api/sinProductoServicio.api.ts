...
import {SinMotivoAnulacionProps, SinProductoServicioProps} from "../interfaces/sin.interface";
 ...
 */
export const fetchSinProductoServicioPorActividad = async (codigoActividad: string): Promise<SinProductoServicioProps[]> => {
    const client = new GraphQLClient(import.meta.env.ISI_API_URL)
    const token = localStorage.getItem(AccessToken)
    // Set a single header
    client.setHeader('authorization', `Bearer ${token}`)

    const data: any = await client.request(queryProductoServicioPorActividad, {codigoActividad: codigoActividad || ''})
