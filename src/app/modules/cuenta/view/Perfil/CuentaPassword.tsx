...
const CuentaPassword: FunctionComponent<Props> = (props) => {
    ...
    const formik: FormikProps<UsuarioCambiarPasswordInputProps> = useFormik<UsuarioCambiarPasswordInputProps>({
        initialValues: {
            password: '',
            nuevoPassword: '',
            nuevoPassword2: ''
        ...
        onSubmit: async (values) => {
            await swalAsyncConfirmDialog({
                preConfirm: () => {
                    return apiUsuarioCambiarPassword(values)
                        .catch(err => {
                            swalException(err)
                            return false
                        })
                ...
                if (resp.isConfirmed) {
                    notSuccess("Se ha cambiado la contraseña correctamente, esta se aplica a partir del proximo reinicio de sesión")
                }
            })
        }
    });
                            ...
                            <Grid item xs={12}>
                                <FormControl fullWidth>
                                    <TextField
                                        ...
                                        size="small"
                                        onChange={formik.handleChange}
                                        error={formik.touched.nuevoPassword2 && Boolean(formik.errors.nuevoPassword2)}
                                        helperText={formik.touched.nuevoPassword2 && formik.errors.nuevoPassword2}
                                    />
                                </FormControl>
                            </Grid>
