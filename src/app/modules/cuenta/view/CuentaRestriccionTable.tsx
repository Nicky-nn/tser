...
const CuentaRestriccionTable: FunctionComponent<Props> = (props) => {
    ...
    if (isError)
        return <AlertError mensaje={error && error.message}/>
            ...
            isLoading ? <AlertLoading/> :
                restriccion!.map(res => (
                    <React.Fragment key={res.codigo}>
                        <Accordion expanded={expanded === res.codigo} onChange={handleChange(res.codigo)}
                            ...
                            >
                                <Typography>
                                    <strong>SUCURSAL {res.codigo}</strong> / {res.departamento.departamento} - {res.municipio} / {res.direccion} / {res.telefono}
                                </Typography>
                            </AccordionSummary>
                            <AccordionDetails>
                                <Grid container>
                                                    ...
                                                    <React.Fragment key={pv.codigo}>
                                                        ...
                                                        <ListItem
                                                            secondaryAction={
                                                                ...
                                                                <Tooltip title={'¿Cambiar Punto Venta?'}
                                                                         placement={'top'}>
                                                                    <IconButton
                                                                        onClick={() => changePuntoVenta(res.codigo, pv.codigo)}
                                                                        color={'success'} aria-label="select">
                                                        ...
                                                        >
                                                            <ListItemText
                                                                primary={
                                                                    <Typography>
                                                                        <strong>PUNTO
                                                                            VENTA {pv.codigo}</strong> / {pv.nombre} / {pv.tipoPuntoVenta.descripcion}
                                                                    </Typography>
                                                                }
                                                            />
                                                            <Divider/>
