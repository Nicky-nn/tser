...
export const apiUsuarioCambiarPassword = async (input: UsuarioCambiarPasswordInputProps): Promise<Boolean> => {
    const client = new GraphQLClient(import.meta.env.ISI_API_URL)
    const token = localStorage.getItem(AccessToken)
    // Set a single header
    client.setHeader('authorization', `Bearer ${token}`)

    const data: any = await client.request(gqlQuery, {password: input.password, nuevoPassword: input.nuevoPassword})
